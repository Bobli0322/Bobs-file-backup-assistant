...
#Nothing import this module
import tkinter as tk
from tkinter import ttk
from tkinter import messagebox
from tkinter import filedialog
import m_fSync as fsc
import m_Checksum as cks
import m_rDup as rdup
import m_rDirFile as rdf
import threading
import time
...
#        This is to accommodate [fsc 1_7] separating data analysis from processing
#
...
#}0
print('Delimiter: ' + fsc.delim) #delim is in m_gVar which fsc module includes
...
def validArg0(arg1, arg2):#{0
   if arg2 == '':#{1
      if (fsc.delim == '\\' and (not ':' in arg1 or not '\\' in arg1)) or \
         (fsc.delim == '/' and not '/' in arg1):#{2
         return 0
      #}2
      else:#{2
         return 1
   ...
   #}1
      ...
      if arg1 == arg2:#{2
         return 0
      #}2
      elif (fsc.delim == '\\' and (not ':' in arg1 or not '\\' in arg1)) or \
         (fsc.delim == '/' and not '/' in arg1):#{2
         return 0
      #}2
      elif (fsc.delim == '\\' and (not ':' in arg2 or not '\\' in arg2)) or \
         (fsc.delim == '/' and not '/' in arg2):#{2
         return 0
      #}2
      else:#{2
         return 1
...
def validArg(exlArg):#{0
   ...
   #}1
      ...
      if (fsc.delim == '\\' and (not ':' in exlArg or not '\\' in exlArg)) or \
         (fsc.delim == '/' and not '/' in exlArg):#{2
         return 0
      #}2
...
def callback():#{0
   ...
   if thread == 1:#{1
      ret = opResult.get() #return a signal
      #ret = 'Random'
   ...
   #}1
      ...
      if ret != '':#{2
         ...
         dnCounter = int(ret[1])
         cpCounter = int(ret[2]) #count of files to remove
   ...
   #}1
      ...
      ret = opResult.get() #count of items found
   ...
   #}1
      ...
      ret = opResult.get() #count of actions to take
      if ret != 0:#{2
...
def taskF1(srcArg, dstArg, exlSrcArg, exlDstArg):#{0
   global thread
   global opResult
   opResult.put(fsc.backupCopy(srcArg, dstArg, exlSrcArg, exlDstArg))
   #time.sleep(5)
   thread = 1
#}0
def taskF11(iMode1, iCS):#{0
   global thread
   fsc.filing(iMode1, iCS)
   thread = 11
#}0
...
def taskF2(tarArg, iMode2, iCS, exl2Arg):#{0
   ...
   global opResult
   if iMode2 == 1:#{1
      opResult.put(rdup.globalCompare(tarArg, iCS, exl2Arg))
   #}1
   elif iMode2 == 2:#{1
      opResult.put(rdup.localCompare(tarArg, iCS, exl2Arg))
   #}1
   #time.sleep(10)
   thread = 2
#}0
...
def taskF3(tarArg, nameArg, typeArg3, exl3Arg):#{0
   global thread
   global opResult
   opResult.put(rdf.delDirFile(tarArg, nameArg, typeArg3, exl3Arg))
   #time.sleep(5)
   thread = 3
#}0
def taskF33():#{0
