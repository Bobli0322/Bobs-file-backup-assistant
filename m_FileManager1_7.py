...
def filing(iMode, iCS):#{0
    ...
    if iMode == 3 or iMode == 1:#{1
        ...
        for cpf in checkNcopyList:#{2
            ...
            #Following line does not consider if the aged file is renamed
            if iMode == 1 and tempDst in agedFileList:#{3
                continue
            #}3
            else:#{3
                #print('Folder copy ' + cpf[0] + ' to ' + cpf[1])
                if fOps.checkNcopy(cpf[0], cpf[1], cpf[2], cpf[3], delim) == True:#{4
    ...
    #}1
        ...
        if len(dstFileList) != 0:#{2
            ...
            #}3
                ...
                for c in range(len(dstFileList)):#{4
                    toRemove = True
                    dstDirFile = func.buildPath(dstDir, dstFileList[c][1], delim)
                    dstFile = dstFileList[c][1]
                    dstFsize = dstFileList[c][2]
                    #Assume if file size different then content different
                    if dstFsize in srcSizeList:#{5 
                        for d in range(len(srcFileList)):#{6
                            srcFile = srcFileList[d][1]
                            ...
                            if dstFsize == srcFsize:#{7
                                ...
                                if fCompare(dstDirFile, srcDirFile, cmp_Mode):#{8
                                    ...
                                    dstFileDateLst = dstFileDateTxt.split(' ')
                                    dstFileDateStr = dstFileDateLst[len(dstFileDateLst)-1]
                                    dstFileDateInt = int(dstFileDateStr) #assign dstFile access year(int)
                                    fileAgeA = nowYr - dstFileDateInt
                                    #default value of fileAge is 2, meaning max of 3 years
                                    #before the file is re-copied
                                    if fileAgeC <= 2 or fileAgeA <= 2:#{9
                                        ...
                                        #}10
                                        toRemove = False
                                        sameFileSrc.append(srcFileList[d]) #???
                                        break
                                    #}9
                                    #file is older than 3 years
                                    ...
                                    #because aged file is removed, and new src file copied
                                        ...
                                        agedFile = agedFile + 1
                                        agedFileList.append(dstDirFile)
                                        break
                                    #}9
                                #}8
                            #}7
        ...
        #}2
            ...
            #}3
                ...
                #srcToCopy = list(set(sameFileSrc)^set(srcFileList)) #remove same elements between 2 lists
...
def backupCopy(srcDir, dstDir, exlSrcDir, exlDstDir):#{0
    ...
    #}1
        ...
        if not dstDir in edList and not srcDir in esList:
